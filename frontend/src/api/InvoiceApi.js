...
class InvoiceApi {
    ...
    static show (id, config) {
        ...
        return axios.get(`http://localhost:8080/api/invoice/${id}`, config);
    }
    // GET: /api/invoice
    ...
    static store (data, config) {
        // return new Promise((resolve)=>{resolve();});
        return axios.post('http://localhost:8080/api/invoice', data, config);
    }
    // PUT: /api/invoice/:id  // TODO: create endpoint
    static update (id, data, config) {
        return new Promise((resolve)=>{resolve();});
        // return axios.put(`http://localhost:8080/api/invoice/${id}`, data, config);
    }
    // DELETE: /api/invoice/:id
    static deleteData (id, config) {
        // return new Promise((resolve)=>{resolve();});
        return axios.delete(`http://localhost:8080/api/invoice/${id}`, config);
    }
}
